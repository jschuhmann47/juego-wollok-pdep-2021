...
//		posParedes.addAll([new Position(x=3,y=10)])
		...
		posParedes.addAll([new Position(x=11,y=12), new Position(x=12,y=12), new Position(x=13,y=12)])
		
		posParedes.forEach { p => list_paredes.add(new Pared(position = p)) }	
		list_paredes.forEach { p => self.dibujar(p) }	
		list_paredes.forEach({x => game.whenCollideDo(x, { objeto => objeto.colisionPared() } ) } )
		
		game.onTick(1200, "movimiento", { enemigo.perseguir() })
	}
